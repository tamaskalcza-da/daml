...
object Ledger {
  ...
  type ScenarioContractId = ContractIdString

  object ScenarioNodeId {
    def apply(acoid: AbsoluteContractId): ScenarioContractId = acoid.coid
  ...
  case class Ledger(
    ...
    def lookupGlobalContract(
      ...
      ledgerData.coidToNodeId
        .get(coid)
        .flatMap(ledgerData.nodeInfos.get) match {
  ...
  final case class LedgerData(
    ...
    def nodeInfoByCoid(coid: AbsoluteContractId): NodeInfo =
                  ...
                  node match {
                    ...
                    case ex: NodeExercises.WithTxValue[ScenarioNodeId, AbsoluteContractId] =>
                      ...
                      val newCache1 =
                        if (ex.consuming) {
                          val newCache0_1 =
                            newCache0.markAsInactive(ex.targetCoid)
                          val nc = newCache0_1
